...
int w_th_seq_req(struct sip_msg* req)
{
	struct sip_uri ruri;
	int i;
	struct dlg_cell* dlg;
	...
	for(i = 0 ; i< ruri.u_params_no ; i++) {
		if(ruri.u_name[i].len == 3 && memcmp(ruri.u_name[i].s, "did", 3)==0)
			break;
	}
	if(i == ruri.u_params_no) {
		LM_ERR("Not a topology hiding request - did param not found in RURI\n");
		return -1;
	}

	if ( parse_dlg_rr_param( ruri.u_val[i].s, ruri.u_val[i].s+ruri.u_val[i].len, &h_entry, &h_id)<0 ) {
		LM_ERR("did in RURI not well formed\n");
		return -1;
	}

	/* skip initial requests */
	...
	if (dlg==0) {
		LM_DBG("unable to find dialog for %.*s "
			...
			ruri.u_val[i].len, ruri.u_val[i].s);
		/*TODO - check dialog after SIP info? if it doesn't have RURI param */
	} else {
		...
		if (match_dialog(dlg,&callid,&ftag,&ttag,(unsigned int*)&dir, (unsigned int*)&dst_leg )==0){
	...
	dlg_handle_dlg_onroute(req, dlg, dir, dst_leg);
